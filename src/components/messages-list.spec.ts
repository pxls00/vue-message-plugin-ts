...
describe('Messages List Component:', () => {
  ...
  it('check every message items renderable paramters is contains in html', () => {
    ...
    messageItems.forEach(itemEl => {
      ...
      if(itemEl.attributes('data-message-type') === 'wait') {
        ...
        if(DefaultMessageWaitData.class && DefaultMessageWaitData.class.length) {
  ...
  it('check is remove message is working', async () => {
    ...
    expect(messageStore.$state.messages.includes(messageErrorItem)).not.toBe(true)
  ...
  it('check is adding new message is working', async () => {
    const newMessage = JSON.parse(JSON.stringify(messageList.find(item => item.type === 'success'))) as MessageItem
    newMessage.id = 123
    ...
    const messageItem = wrapper.findAll(`[data-testid="message__item"][data-message-type="${newMessage.type}"]`)
    ...
    const newMesssageItem = wrapper.findAll(`[data-testid="message__item"][data-message-type="${newMessage.type}"]`)
    expect(newMesssageItem.length).toBe(1)
    expect(messageStore.messages.filter(item => item.type === newMessage.type).length).toBe(1)
  })
